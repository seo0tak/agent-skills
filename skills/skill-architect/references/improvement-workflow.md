...
사용자가 개선 이력이나 장기 검토 재개를 원할 때 읽는다. 단회 감사에는
저장소를 만들지 않아도 된다. 이 절차는 발견과 개선 결과를 연결하는 로컬
기록 방식이다. 자동 학습·자동 실행 플랫폼이 아니며 모델 자체를 학습시키지 않는다.

## 기록과 실제 작업의 경계

`scripts/improvement_tracker.py`는 Python 3 표준 라이브러리만 사용한다.
모든 명령은 사용자가 지정한 로컬 저장소의 JSON 기록만 작성하거나 읽는다.
대상 경로, 후보 변경문, 평가 명령처럼 보이는 문자열, 근거 링크는 **데이터**다.
도우미는 대상 파일이나 링크를 열지 않으며 명령·패치·테스트를 실행하지 않는다.
...
1. `capture`로 문제·재현·검토 맥락을 남긴다. 중단 전 `checkpoint`에 완료한
   ...
   미완료로 둔다. 저장소에는 스킬 개선 자료만 두고 SAST 고객 업무 자료는 넣지 않는다.
2. `proposed`에 관찰한 baseline, 정확한 후보 변경문, 프로젝트 상대 대상 경로와
...
3. 격리된 fixture에서 baseline과 후보를 각각 평가하고 관찰 결과, 근거, 기존
   동작 회귀 결과를 `evaluated`로 남긴다. 평가 실행 자체는 별도 작업이다.
...
도우미는 입력된 SHA256과 결과의 일치만 검사한다. 파일이 실제로 바뀌었는지,
테스트를 실행했는지 또는 근거가 사실인지는 별도로 확인해야 한다.

## CLI와 저장 계약

명령 경로는 스킬 루트 기준이다. `--store`는 명시적으로 고른 프로젝트별 로컬
디렉터리이며 기본값·전역 저장소는 없다. `--data`는 사용자가 준비한 JSON 파일이다.
...
{"schemaVersion":"1.0","id":"empty-input","revision":1,"events":[
  {"action":"captured","at":"2026-09-09T09:00:00+09:00","data":{
    ...
    "reproduction":"격리된 fixture에서 빈 SKILL.md를 감사한다.",
...
허용하지 않는 프로젝트 상대 POSIX 경로다. 도우미가 이를 파일 경로로 사용하지는 않는다.
...
쓰기에는 저장소당 `.write-lock` 디렉터리를 배타적으로 만들고, 고유 임시 파일에
...
자동 삭제하지 않는다. 작성 중인
프로세스가 없음을 확인하고 기존 JSON을 검증한 뒤 그 lock만 복구자가 제거한다.
...
스킬 루트에서 임시 저장소에 복사해 재개 요약을 볼 수 있다.
...
독립 평가나 반복 압박 테스트를 하지 않았다면 그렇게 적고 합성 fixture 성공을
실전 품질·다섯 번의 모델 테스트 성공으로 확대하지 않는다.

최종 보고는 결과·검증과 한계·미완료 작업·다음 행동 순서다. 예: “빈 입력 후보는
재현을 해결했지만 정상 입력 회귀가 실패하여 적용 전 상태다. 다음은 후보 수정과
동일 사례 재평가다.” 기록 파일 경로·revision을 연결해 다음 세션에서 이어간다.
