...
- python3 tools/sast_state.py status로 미완료 체크포인트와 검증 신선도를
  확인합니다. 손상이나 중간 수정이 있으면 prompts/07 절차부터 수행합니다.
...
- "연속으로", "N개 그룹", "끝까지" 요청이면 그 범위 내에서 그룹 사이에
  ...
  멈춥니다. 정책 질문은 해당 항목을 파킹하고 독립적인 항목을 계속할 수 있습니다.
  파킹 질문이 5개 모이거나 범위를 완료하면 중복 질문을 합쳐 하나씩 제시합니다.
...
- 등록된 sast-bulk-worker는 기계적 집계·경량 가이드·증적 행을,
  sast-remediator는 승인된 가이드 범위의 수정을 맡을 수 있습니다.
  결론·정책·verified 승격은 메인이 확인합니다. 해당 에이전트가 없으면
  메인이 같은 절차를 직접 수행하며, 실행 성능이나 비용이 같다고 가정하지 않습니다.
...
2. 현재 소스와 선택 보고서의 검출 코드를 대조합니다. 항목별
   ...
   동일 원인의 멤버는 groupGuideRef, summary, delta를 쓸 수 있지만,
...
4. 승인된 범위에서 기존 동작을 보존하는 최소 수정을 합니다.
   ...
   있을 때만 baseline-default로 기록해 관찰 장치를 추가합니다.
