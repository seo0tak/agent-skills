...
- data/input-validation.json에서 이번 게이트가 확정한 입력 세트의 SAST 자료
  (벤더 모드: PDF 1개 이상 + 스프레드시트 1개 / SARIF 모드: .sarif 1개, PDF 선택)
...
수행할 작업:
1. 확정된 입력 검증 결과의 세트·manifest 경로를 사용하고 프로젝트·보고서
   식별정보를 유지합니다. 파일명이나 수정 시각으로 다른 “최신” 세트를 다시 고르지 않습니다.
...
3. 검출 목록을 표준 finding 형식으로 변환합니다.
   - 벤더 모드: 스프레드시트의 전체 검출 항목 사용.
     ...
     의미가 불명확한 컬럼은 추측으로 결론에 반영하지 말고 값을
     `report.vendorAttributes`에 원문 그대로 보존한 뒤(예:
     `{"A.S": "Y"}`) 값 분포를 보고하고 사용자에게 의미를 확인합니다.
     보존해 두면 의미가 확인됐을 때 재분류할 수 있습니다.
     `A.S`처럼 의미가 불명확한 벤더 필드는 해당 보고서·버전에서 의미를
