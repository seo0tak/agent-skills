...
### 1.12.3 → 1.12.4: 판정 근거와 답변 추출 보강

- schemaVersion 1.0, JSON 필드·enum, 기존 결과·백업의 읽기 형식은 유지한다.
  규칙 적용 여부·현재 영향·조치 필요성은 기존 근거 필드에 구분해 기록한다.
  새 지침만으로 과거 오탐·예외·완료 기록을 자동 재분류하지 않는다. 근거가
  부족한 항목은 원문을 보존하고 현재 소스에서 개별 재검토한다.
- 답변 추출은 `조치 결론:`과 선택 라벨 `작업 상태:`를 독립적으로 읽는다.
  결론은 하나의 정해진 값만 쓰고, 부연 설명은 `판단 이유:`로 옮긴다.
  부정문·복수 결론·잘못된 상태는 추출을 중단하고 기존 결과를 유지한다.
- `작업 상태:`가 빠진 옛 답변의 **새 추출 결과**는 `analyzed`다. `fix`라는
  결론만으로 `change-complete`를 만들던 동작을 바로잡았다. 변경 완료는
  명시한 작업 상태와 비어 있지 않은 `적용 내용:`을 받아 기록하며,
  `false-positive`에도 가능하다. 실제 변경 근거는 별도로 대조한다.
  텍스트 추출로 `verified`를 만들지 않는다. 기존 파일 결과·백업을 읽을 때의
  상태를 일괄 강등하거나 덮어쓰는 변경은 아니다.
- 위 자산 정책에 따라 대시보드·프롬프트·스키마 설명·문서를 함께 갱신한다.
  사용자 정의 응답 템플릿에는 새 라벨을 대조·병합한다. 설치본이나 진행
  프로젝트 복사본은 저장소 갱신만으로 바뀌지 않는다.

### 1.12.1 → 1.12.2: 소스 탐색 지침·Sparrow 용어 정리

- 데이터 형식·schemaVersion·enum·검증 명령은 바뀌지 않는다. 기존
  `report.vendorAttributes`도 유지하며 데이터 이전은 필요하지 않다.
- 위 자산 정책에 따라 지침과 배포 사본을 갱신한다. 진행 중 프로젝트의
  사용자 수정·업무 상태는 보존한다. 다음 소스 검토/재개 때 새 탐색 규칙으로
  사용 가능한 도구와 색인의 최신성을 확인한다. LSP 설치를 요구하는 변경이 아니다.
