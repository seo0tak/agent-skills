...
- 대상 프로젝트의 현재 작업 사본(이하 "현재 소스")이 동작의 기준이다.
  ...
  검출 구문을 다시 찾는다.
- 검출 규칙의 적용 여부, 현재 영향과 안전 조건, 조치 필요성을 구분한다.
  큰 피해가 확인되지 않았다는 이유만으로 오탐으로 분류하지 않는다.
  결론과 작업 단계는 독립적으로 기록한다. 오탐에 예방 수정을 했어도
  결론은 오탐이고, 수정 제안만 했다면 변경 완료가 아니다. 상세 판정·기록은
  `SECURITY_SAST_WORKFLOW.md`의 5단계와 항목별 응답 형식을 따른다.
