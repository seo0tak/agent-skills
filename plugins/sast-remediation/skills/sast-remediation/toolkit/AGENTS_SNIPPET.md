...
- 경로·텍스트·심볼·이력 탐색, 호출부 확인, 수정 후 진단과 재개 시 인덱스
  재확인은 `SECURITY_SAST_WORKFLOW.md`의 「소스 탐색 계약」을 따른다.
  현재 환경에 노출되고 최신 파일을 반영해 정상 응답하는 의미 탐색만 우선하며,
  실패·참조 0건을 영향 없음의
  증거로 삼지 않는다. 사용할 수 없으면 텍스트 탐색·실제 소스·컴파일/린트/테스트로
  대체하고 중요한 불확실성을 알린다.
