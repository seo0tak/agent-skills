...
5. 수정 후 checkpoint --phase after-edit로 현재 판단과 다음 작업을 저장합니다.
   ...
   신뢰할 수 없으면 컴파일러·타입 검사·린트로 확인합니다. 이번 변경으로 생긴
   타입·import·함수 계약 오류는 승인 범위 안에서 다음 변경 전에 해결합니다.
   범위 밖 기존 경고를 정리 작업으로 확대하지 않습니다. 새 오류를 해결하지
   못하면 검증 성공으로 보고하지 않고 한계를 기록합니다. 진단 0건만으로
   verified 후보로 올리지 않습니다.
