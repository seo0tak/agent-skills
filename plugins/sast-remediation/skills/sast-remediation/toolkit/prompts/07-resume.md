...
2. data/progress.json으로 전체 진행 현황을 파악합니다
   (상태별·결론별 건수).
3. project/WORK_GROUPS.md에서 그룹별 완료 여부를 확인하고,
   security-results/index.json과 대조해 미완료 그룹을 찾습니다.
4. project/DECISION_LOG.md와 PROJECT_SECURITY_POLICY.md를 읽어
   이미 확정된 정책을 파악합니다. 확정된 정책은 다시 묻지 않습니다.
5. 직전 세션이 남긴 미해결 정책 질문이 progress나 결정 로그에 있으면
   그것부터 사용자에게 확인합니다.
...
큰 findings.json은 전체를 다시 읽지 말고 이번에 다룰 작업 그룹의
항목만 조회해 사용합니다.
```
