...
- PDF/SARIF의 해결 예시를 그대로 붙이지 않는다. 현재 소스의 실제 타입,
  호출부, 프레임워크에 맞게 적용한다.
- 필수 조치인지 선택한 예방 조치인지 가이드에서 확인하고, 실제 적용 내용을
  구분해 보고한다. 예방 수정을 했다는 이유로 원래 오탐 결론을 수정 결론으로
  바꾸지 않는다. 최종 결론은 총괄 AI가 근거를 대조해 결정한다.
...
- 변경을 실제로 마친 경우에만 workflowStatus를 change-complete로 기록한다.
  수정 제안이나 중간 작업을 변경 완료로 보고하지 않는다. 검증이 통과하지
  않았으면 그 상태와 한계를 남기며, verified 승격은 총괄 AI가 결정한다.
